...
public class Task_18092024_008 {
    public static void main(String[] args) {
       ...
       long b = a; //implicit casting
        ...
        long course1 = 100;
        float gst1 = 18.5f;
        float result3 = course1+gst1; // float has upper hand due to its fractional acceptance
        // even though it is 32 bit java accepts float precision compare to long 64 bit
        float result4 = (float)course+gst;

        // example 2:
        float price_full = 100;
        double price_gst = 18.5;
        //  float total = price_full+price_gst; not acceptable implicit narrowing
        double result5 = price_full+price_gst; // widening implicit
        double result6 = (double)price_full+price_gst; // widening implicit
        float result7 = price_full+ (float)price_gst; // narrowing explicit

        System.out.println("double implicit:" +result5);
